...
fn setup_vector_graphics(mut commands: Commands) {
    ...
    let rust_logo_svg_path_drawing = generator.path::<f32>(rust_logo_svg_path.clone());
    let bbox_path = PathTransformer::new(rust_logo_svg_path.clone());
    let bbox = bbox_path.to_box(None);
    let mut scene = vello::Scene::new();

    rust_logo_svg_path_drawing.draw(&mut scene);
    ...
    commands.spawn(VelloSceneBundle {
        ...
        transform: Transform::from_translation(Vec3::new(
            -(bbox.width() as f32) / 2.0,
            (bbox.height() as f32) / 2.0,
            0.0,
        )),
        ..default()
    });
